...
// ----------------------------------------------------------------------------
// RMSNorm kernels and launchers

// RMSNorm forward kernel: out = (x * inv_rms) * weight + bias
__global__ void rmsnorm_forward_kernel(floatX* __restrict__ out,
                                       float* __restrict__ dummy_mean,
                                       float* __restrict__ inv_rms_out,
                                       const floatX*  __restrict__ inp,
                                       const floatX*  __restrict__ weight,
                                       const floatX* __restrict__ bias,
                                       int N, int C) {
    assert(blockDim.x == WARP_SIZE);
    extern __shared__ char* params[];
    x128* s_weight = reinterpret_cast<x128*>(params);
    ...
    for(int c = threadIdx.x * x128::size; c < C; c += WARP_SIZE * x128::size) {
        const x128 in_data = load128cs(inp + c);
        for(int k = 0; k < x128::size; ++k) {
            float v = (float)in_data[k];
            sumsq += v * v;
        }
        s_in[c / x128::size] = in_data;
    }
    sumsq = warpReduceSum(sumsq);
    float inv_rms = rsqrtf(sumsq / C + eps);
    if(threadIdx.x == 0) {
        if (inv_rms_out != nullptr) { inv_rms_out[idx] = inv_rms; }
        if (dummy_mean != nullptr) { dummy_mean[idx] = 0.0f; }
    }
    ...
    for(int c = threadIdx.x * x128::size; c < C; c += WARP_SIZE * x128::size) {
        ...
        x128 out_data;
        for(int k = 0; k < x128::size; ++k) {
            float n = inv_rms * (float)in_data[k];
...
__global__ void fused_residual_rmsnorm_forward_kernel(floatX* residual, floatX* normed,
                                                      float* dummy_mean, float* inv_rms_out,
                                                      const floatX* inp1, const floatX* inp2,
                                                      const floatX* weight, const floatX* bias,
                                                      int N, int C) {
    assert(blockDim.x == WARP_SIZE);
    ...
    for(int c = threadIdx.x * x128::size; c < C; c += WARP_SIZE * x128::size) {
        ...
        x128 res;
        for(int k = 0; k < x128::size; ++k) {
            float v = (float)in1[k] + (float)in2[k];
            res[k] = v;
            sumsq += v * v;
        }
        store128cs(residual + c, res);
        s_res[c / x128::size] = res;
    }
    sumsq = warpReduceSum(sumsq);
    float inv_rms = rsqrtf(sumsq / C + eps);
    if(threadIdx.x == 0) {
...
rmsnorm_backward_kernel(floatX* dinp, floatX* dweight, floatX* dbias, float* scratch,
                        const floatX* dout, const floatX* inp, const floatX* weight,
                        const float* dummy_mean, const float* inv_rms_vec,
                        int B, int T, int C) {
    int BLOCK_SIZE = blockDim.x;
    int warpsInBlock = BLOCK_SIZE / WARP_SIZE;
    extern __shared__ float shared[];
    ...
    for (int bt = baseIdx; bt < B * T; bt += warpsInGrid) {
        ...
        float s1 = 0.0f; // sum_j (u_j * x_j), u_j = g_j * w_j
        for (int i = warpThreadIdx * x128::size; i < C; i += WARP_SIZE * x128::size) {
            x128 dout128_i   = load128(dout_bt + i);
            x128 inp128_i    = load128(inp_bt  + i);
            x128 weight128_i = load128(weight  + i);
            for (int k = 0; k < x128::size; k++) {
                float u = (float)weight128_i[k] * (float)dout128_i[k];
                s1 += u * (float)inp128_i[k];
            }
        }
        const float inv_rms = inv_rms_vec[bt];
        ...
        for (int c = 0; c < iterations_C; c++) {
            ...
            for(int o = 0; o < x128::size / f128::size; ++o) {
                ...
                for(int i = 0; i < f128::size; ++i) {
                    int x = o * f128::size + i;
                    float g = (float)dout128[x];
                    float w = (float)weight128[x];
                    float xx = (float)inp128[x];
                    float n = inv_rms * xx;
                    dbias_f[i] = g;
                    dweight_f[i] = n * g;
                    float u = w * g;
                    float dval = inv_rms * u - inv_rms3 * (xx / (float)C) * s1;
                    dinp128[x] = (floatX)((float)dinp128[x] + dval);
                }
                if (warpId != 0) {
                    store128(dbias_tmp_shared + threadIdx.x * f128::size, dbias_f);
    ...
    if (*tmp_flag == gridDim.x-1) {
        ...
        // convert from float to floatX and add to global grads
        for (int c = warpId; c < iterations_C; c += warpsInBlock) {
            int global_index = (warpThreadIdx * x128::size) + (c * C_per_iteration);
            if (global_index >= C) { break; }
            x128 dbias128 = load128(dbias + global_index);
            x128 dweight128 = load128(dweight + global_index);
            for(int o = 0; o < x128::size / f128::size; ++o) {
                f128 s_db = load128(dbias_shared + global_index + o * f128::size);
...
void rmsnorm_forward(floatX* out, float* dummy_mean, float* inv_rms,
                     floatX* inp, const floatX* weight, const floatX* bias,
                     int B, int T, int C, cudaStream_t stream) {
    NVTX_RANGE_FN();
    const int block_size = 256;
    ...
    cudaCheck(cudaGetLastError());
    if (status == cudaSuccess) {
        rmsnorm_forward_kernel<<<grid_size, dim3(WARP_SIZE, block_y), smem, stream>>>(out, dummy_mean, inv_rms, inp, weight, bias, N, C);
    } else {
        int block_y_fb = 1;
        int grid_size_fb = CEIL_DIV(N, block_y_fb);
        size_t smem_fb = (2 + block_y_fb) * C * sizeof(floatX);
        rmsnorm_forward_kernel<<<grid_size_fb, dim3(WARP_SIZE, block_y_fb), smem_fb, stream>>>(out, dummy_mean, inv_rms, inp, weight, bias, N, C);
    }
    cudaCheck(cudaGetLastError());
}

void fused_residual_rmsnorm_forward(floatX* residual, floatX* normed, float* dummy_mean, float* inv_rms,
                                    const floatX* inp1, const floatX* inp2,
                                    const floatX* weight, const floatX* bias,
                                    int N, int C, cudaStream_t stream) {
    const int block_size = 256;
    ...
    size_t smem = (2 + block_y) * C * sizeof(floatX);
    cudaCheck(cudaGetLastError());
    auto status = cudaFuncSetAttribute(fused_residual_rmsnorm_forward_kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem);
    cudaCheck(cudaGetLastError());
    if(status == cudaSuccess) {
        fused_residual_rmsnorm_forward_kernel<<<grid_size, dim3(WARP_SIZE, block_y), smem, stream>>>(residual, normed,
                                                                                                     dummy_mean, inv_rms, inp1, inp2,
                                                                                                     weight, bias, N, C);
    } else {
        residual_forward(residual, inp1, inp2, N*C, stream);
        rmsnorm_forward(normed, dummy_mean, inv_rms, residual, weight, bias, N, 1, C, stream);
    }
    cudaCheck(cudaGetLastError());
}

void rmsnorm_backward(floatX* dinp, floatX* dweight, floatX* dbias, float* scratch,
                      const floatX* dout, const floatX* inp, const floatX* weight, const float* dummy_mean, const float* inv_rms,
                      int B, int T, int C, cudaStream_t stream) {
    NVTX_RANGE_FN();
    const int block_size = 512;
    ...
    rmsnorm_backward_kernel<<<grid_size, block_size, shared_mem_size, stream>>>(dinp, dweight, dbias, scratch, dout, inp, weight, dummy_mean, inv_rms, B, T, C);
    cudaCheck(cudaGetLastError());
}
